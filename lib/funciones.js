...
function cargarDatosRRHH_t() {
	...
	var html = '<div data-role="content">';
		...
		jQuery.get("http://corprocessu.com:8000/selectRoles",function(resultado2){
		data = jQuery.parseJSON(resultado);
...
function graficoG(gasto){	
	...
	progress1.Set('chart.numticks', 5); // divisiones
	progress1.Set('chart.gutter.left', 10); // margen izq
	progress1.Set('chart.shadow', true); // sombra
...
function graficoRendimiento(rendimiento,desc){
	...
	hprogress2.Set('chart.gutter.left', 10);
	hprogress2.Set('chart.margin', 5);
	hprogress2.Set('chart.shadow', true);
...
function graficoRT(rt){
	...
	cg.Set('chart.value.text.boxed', false); <!-- Cuadro Borde -->
	cg.Set('chart.value.text.size', 8);
	cg.Set('chart.value.text.font', 'Arial');
	cg.Set('chart.value.text.bold', true);
	cg.Set('chart.value.text.decimals', 0);
	cg.Set('chart.gutter.left', 0);
	cg.Set('chart.radius', 130); // tam graf
	//cg.Set('chart.centerpin.stroke', 'red'); // brode aguja
	//cg.Set('chart.centerpin.fill', 'red'); // relleno aguja
	cg.Set('chart.centerpin.color', 'blue'); // relleno con gradiente
	cg.Draw();
}
...
function graficoBT(hp,hpex,hc){
	...
	r1.Set('chart.centerx', 130);
	r1.Set('chart.centery', 140);
	r1.Set('chart.tickmarks.small.num', 100);
	r1.Set('chart.tickmarks.big.num', 10);
	r1.Set('chart.radius', 125);
	//r1.Set('chart.title', 'titulo');
	//r1.Set('',);
	r1.Set('chart.shadow',true);
	r1.Draw();
